  ...
  return (
    <section id="tickets" className="section-space px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl"><div className="relative overflow-hidden rounded-[2rem] border border-navy/10 bg-white/80 px-6 py-9 shadow-paper sm:px-10"><div className="section-floral opacity-70" />
        <div className="relative z-10"><div className="flex items-start justify-between gap-6"><div><p className="eyebrow">Reserve your seat</p><h2 className="mt-4 font-display text-4xl text-ink sm:text-5xl">Choose your three drinks.</h2></div><div className="relative mr-10 shrink-0 pt-1 text-right">{availabilityLoading ? <p className="flex items-center justify-end gap-2 pt-3 text-sm text-navy/55"><LoaderCircle className="h-4 w-4 animate-spin" />Checking availability</p> : <>{hasDiscount ? <p className="font-display text-xl text-navy/45 line-through decoration-navy/45">$69.99</p> : null}<p className="font-display text-4xl text-ink">{hasDiscount ? "$55.99" : "$69.99"}</p><p className="mt-1 text-[10px] uppercase tracking-[0.18em] text-navy/55">per guest · CAD</p>{hasDiscount ? <span className="absolute -right-14 top-7 rotate-12 rounded-full bg-rose-500 px-3 py-2 text-[10px] font-semibold tracking-[0.12em] text-white shadow-sm">−20%</span> : null}</>}</div></div>
